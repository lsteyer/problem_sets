...
```{r}
# creating new dataframe, filtered_d, that has filtered out rows with "1" as their value in col 'DoNotUse'
# your code here: exclude subjects that are marked as "DoNotUse"

filtered_d = d %>% 
  filter(is.na(d$DoNotUse))
# used head(filtered_d) and tail(filtered_d) to quickly check my work
```
...
```{r TamirFig1}
ggplot(rating_summary_d, aes(x=IsRecall, y=MeanRating, fill=Emotion)) +
